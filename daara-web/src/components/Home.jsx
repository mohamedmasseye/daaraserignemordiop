...
const PWAInstallGuide = ({ isOpen, onClose }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[150] bg-white flex flex-col">
      ...
      <div className="bg-gray-50 p-4 shrink-0">
        <div className="max-w-md mx-auto flex bg-white p-1 rounded-2xl border border-gray-100 shadow-sm">
          <button onClick={() => { setBrowser('safari'); setStep(0); }} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${browser === 'safari' ? 'bg-primary-900 text-white shadow-lg' : 'text-gray-400'}`}>Safari (iOS)</button>
          <button onClick={() => { setBrowser('google'); setStep(0); }} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${browser === 'google' ? 'bg-primary-900 text-white shadow-lg' : 'text-gray-400'}`}>Chrome (Android)</button>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto p-4 md:p-10 flex flex-col items-center">
        <div className="max-w-2xl w-full flex flex-col items-center text-center">
          <motion.div key={`${browser}-${step}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <div className="inline-flex items-center gap-2 bg-gold-50 text-gold-600 px-4 py-1.5 rounded-full text-[10px] font-black uppercase mb-4">{INSTALL_STEPS[browser][step].icon} {INSTALL_STEPS[browser][step].title}</div>
            <h4 className="text-xl md:text-3xl font-serif font-bold text-primary-900">{INSTALL_STEPS[browser][step].desc}</h4>
          </motion.div>
          <div className="w-full max-w-[280px] md:max-w-[350px] aspect-[9/16] bg-gray-100 rounded-[3rem] border-[8px] border-primary-900 shadow-2xl overflow-hidden relative mb-10">
             <motion.img key={`img-${browser}-${step}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} src={INSTALL_STEPS[browser][step].img} className="w-full h-full object-cover" onError={(e) => { e.target.src = "https://via.placeholder.com/600x1200?text=Capture+Installation"; }} />
          </div>
        </div>
      </div>
      <div className="p-6 bg-white border-t border-gray-100 shrink-0">
        <div className="max-w-md mx-auto flex items-center justify-between gap-6">
          ...
          {step < 2 ? (<button onClick={() => setStep(s => s + 1)} className="flex-1 py-4 bg-primary-900 text-white rounded-2xl font-black uppercase text-xs shadow-xl flex items-center justify-center gap-2">Suivant <ChevronRight size={18}/></button>) : (<button onClick={onClose} className="flex-1 py-4 bg-gold-500 text-primary-900 rounded-2xl font-black uppercase text-xs shadow-xl">Terminer</button>)}
        </div>
      </div>
    </motion.div>
  );
...
const PdfPopup = ({ url, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-primary-950/95 backdrop-blur-xl">
      ...
      <div className="md:hidden p-3 bg-primary-950 border-t border-white/5 text-center text-[10px] text-primary-400 font-bold uppercase">Scrollez pour lire la suite</div>
    </div>
  );
};
...
export default function Home() {
  ...
  // ✅ LOGIQUE DE DÉTECTION PWA / LAPTOP / MEMOIRE
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (upcoming[0]) {
            ...
            if (log.eventId !== upcoming[0]._id) setTimeout(() => setShowPopup(true), 2500); 
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 overflow-x-hidden">
      ...
      {/* 1. BANNIÈRE PWA (MOBILE SEULEMENT) */}
      <AnimatePresence>
        {showInstallBanner && (
          <motion.div initial={{ y: -100 }} animate={{ y: 0 }} exit={{ height: 0, opacity: 0 }} className="bg-primary-950 text-white py-3 px-4 flex items-center justify-between border-b border-white/5 relative z-[60] shadow-2xl overflow-hidden">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left"><h2 className="text-[11px] md:text-sm font-black uppercase tracking-tighter leading-none">Daara App</h2><p className="text-[9px] text-primary-400 font-bold uppercase mt-1 tracking-widest">Version pour Téléphone</p></div>
            </div>
            <div className="flex items-center gap-3">
      ...
      {/* 2. POPUPS (CONDITIONNELS) */}
      <AnimatePresence>{showPopup && featuredEvent && <EventPopup event={featuredEvent} onClose={() => setShowPopup(false)} onBook={() => { setShowPopup(false); navigate('/evenements'); }} />}</AnimatePresence>
      ...
      <section id="about" className="py-24 px-4 md:px-8 max-w-7xl mx-auto">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={{ visible: { transition: { staggerChildren: 0.2 } } }} className="grid md:grid-cols-2 gap-12 items-center">
          <motion.div variants={fadeInUp} className="relative">
            ...
            <div className="relative rounded-[3rem] overflow-hidden shadow-2xl h-[500px] bg-primary-900 flex items-center justify-center">
              {getSecureUrl(content.about.image) ? <img src={getOptimizedImage(getSecureUrl(content.about.image), 600)} alt="" className="w-full h-full object-cover opacity-90" /> : <ImagePlaceholder label="Portrait" />}
      ...
      {/* 5. LES PILIERS (MÉDIATHÈQUE / AGENDA) */}
      <section className="bg-primary-900 text-white py-24 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="mb-16"><h2 className="text-3xl md:text-5xl font-serif font-bold mb-4 italic">Services & Spiritualité</h2><p className="text-primary-200 text-lg font-light">Explorez les ressources numériques du Daara.</p></motion.div>
            ...
            {['p1', 'p2', 'p3'].map((id) => (
              <motion.div key={id} whileHover={{ y: -10 }} onClick={() => navigate(content.pillars[id]?.link)} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-[2.5rem] p-8 cursor-pointer group text-center flex flex-col items-center">
                <div className="w-full h-48 relative overflow-hidden rounded-[2rem] mb-6 shadow-inner">
                  ...
                  <div className="absolute top-4 right-4 bg-gold-500 text-primary-900 p-2 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity transition-transform scale-50 group-hover:scale-100">{id === 'p1' ? <Calendar size={18}/> : id === 'p2' ? <BookOpen size={18}/> : <PlayCircle size={18}/>}</div>
                </div>
                <h3 className="text-2xl font-serif font-bold mb-2 group-hover:text-gold-400 transition-colors">{content.pillars[id]?.label}</h3>
                <p className="text-primary-200 text-sm mb-6 leading-relaxed opacity-80">{content.pillars[id]?.desc}</p>
      ...
      <section className="py-24 px-4">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="max-w-5xl mx-auto bg-gold-50 rounded-[3rem] p-12 md:p-20 text-center relative overflow-hidden shadow-2xl border border-gold-100/50">
          ...
          <div className="w-20 h-1 bg-gold-400 mx-auto mt-12 rounded-full"></div>
        </motion.div>
      </section>
      ...
      <footer className="bg-white border-t border-gray-100 py-20 px-4">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-12 text-center md:text-left">
            <div><h4 className="font-black text-[10px] uppercase tracking-[0.2em] text-gold-600 mb-6 flex items-center gap-2 justify-center md:justify-start"><MapPin size={14}/> Adresse</h4><p className="text-primary-900 font-bold whitespace-pre-line leading-relaxed">{content.info.address}</p></div>
            <div><h4 className="font-black text-[10px] uppercase tracking-[0.2em] text-gold-600 mb-6 flex items-center gap-2 justify-center md:justify-start"><Clock size={14}/> Horaires</h4><p className="text-primary-900 font-bold whitespace-pre-line leading-relaxed">{content.info.hours}</p></div>
            <div><h4 className="font-black text-[10px] uppercase tracking-[0.2em] text-gold-600 mb-6 flex items-center gap-2 justify-center md:justify-start"><Calendar size={14}/> Événement Spécial</h4><p className="text-primary-900 font-bold whitespace-pre-line leading-relaxed">{content.info.nextGamou}</p></div>
            <div className="bg-primary-950 p-8 rounded-[2rem] text-white shadow-2xl transform hover:scale-105 transition-transform">
              <p className="text-[10px] font-black uppercase tracking-widest text-gold-500 mb-2">Besoin d'assistance ?</p>
        ...
        <div className="max-w-7xl mx-auto mt-20 pt-8 border-t border-gray-50 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-400 text-[10px] font-bold uppercase tracking-widest">
            <p>© 2026 DAARA SMD - TOUS DROITS RÉSERVÉS</p>
            <div className="flex gap-6"><button className="hover:text-primary-900 transition-colors">Politique</button><button className="hover:text-primary-900 transition-colors">Conditions</button></div>
        </div>
      </footer>
    </div>
  );

...
// --- VARIANTES D'ANIMATION "CLASSE & LUXE" ---
const fadeInUp = {
...
export default function Home() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (upcoming[0]) {
            ...
            if (log.eventId !== upcoming[0]._id || log.lastSeenDate !== new Date().toDateString()) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 overflow-x-hidden">
      ...
      {/* 1. HERO SLIDER (ANIMATION LUXE) */}
      <section className="relative h-[90vh] min-h-[600px] flex items-center justify-center overflow-hidden bg-primary-900">
        <AnimatePresence mode='wait'>
          <motion.div key={content.slides[currentSlide]?.id} initial={{ opacity: 0, scale: 1.15, filter: "blur(20px)" }} animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }} exit={{ opacity: 0 }} transition={{ duration: 2, ease: [0.22, 1, 0.36, 1] }} className="absolute inset-0 z-0">
        ...
        <div className="relative z-10 text-center text-white px-4 max-w-5xl">
           <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.5 }}>
              ...
              <h1 className="text-5xl md:text-8xl font-serif font-bold mb-6 drop-shadow-2xl leading-tight italic">{content.slides[currentSlide]?.title || "Daara SMD"}</h1>
              ...
              <button onClick={() => document.getElementById('about').scrollIntoView({ behavior: 'smooth' })} className="px-10 py-5 bg-gold-500 text-primary-950 rounded-full font-black uppercase tracking-widest text-[10px] shadow-2xl transform hover:scale-105 active:scale-95 transition-all">Explorer le Daara</button>
           </motion.div>
        </div>
      </section>

      {/* 2. BIOGRAPHIE (SCROLL REVEAL) */}
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={staggerContainer} className="grid md:grid-cols-2 gap-16 items-center">
          <motion.div variants={imageReveal} className="relative">
            ...
            <div className="relative rounded-[3rem] overflow-hidden shadow-2xl h-[600px] bg-primary-900 flex items-center justify-center group">
              {getSecureUrl(content.about.image) ? <img src={getOptimizedImage(getSecureUrl(content.about.image), 600)} alt="Portrait" className="w-full h-full object-cover opacity-90 group-hover:scale-110 transition-transform duration-[2.5s] ease-out" /> : <ImagePlaceholder label="Portrait" />}
              <div className="absolute inset-0 bg-gradient-to-t from-primary-950/80 via-transparent to-transparent"></div>
              <div className="absolute bottom-0 left-0 p-12 text-white"><p className="text-gold-400 font-black uppercase tracking-[0.2em] text-xs mb-2">Guide Spirituel</p><h3 className="text-4xl font-serif font-bold italic">Serigne Mor Diop</h3></div>
            </div>
          </motion.div>
          <motion.div variants={fadeInUp} className="space-y-8 text-center md:text-left">
            <h2 className="text-5xl md:text-7xl font-serif font-bold text-primary-900 leading-tight">{content.about.title1}</h2>
            <div className="w-20 h-1 bg-gold-500 mx-auto md:mx-0 rounded-full"></div>
            <p className="text-gray-600 text-xl leading-relaxed font-light italic">{content.about.text1}</p>
            {content.about.bioPdf && (
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsBioOpen(true)} className="inline-flex items-center gap-6 bg-white border-2 border-primary-100 px-10 py-6 rounded-[2.5rem] shadow-xl hover:shadow-2xl transition-all group">
                <div className="text-left"><span className="block font-black text-xs uppercase tracking-widest text-primary-900">Biographie</span><span className="text-[10px] text-gray-400 uppercase font-bold mt-1">Lecture Complète</span></div>
      ...
      {/* 3. LES PILIERS (STAGGERED REVEAL) */}
      <section className="bg-primary-900 text-white py-32 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="text-center mb-20">
            <h2 className="text-4xl md:text-6xl font-serif font-bold mb-6 italic">Services & Spiritualité</h2>
            <p className="text-primary-200 text-xl font-light opacity-70">Explorez les ressources numériques du Daara.</p>
          </motion.div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={staggerContainer} className="grid md:grid-cols-3 gap-10">
            {['p1', 'p2', 'p3'].map((id) => (
              <motion.div key={id} variants={fadeInUp} whileHover={{ y: -15 }} onClick={() => navigate(content.pillars[id]?.link)} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-[3rem] p-10 cursor-pointer group flex flex-col items-center shadow-2xl transition-all duration-500">
                <div className="w-full h-56 relative overflow-hidden rounded-[2.5rem] mb-8 shadow-inner">
                  {getSecureUrl(content.pillars[id]?.image) ? <img src={getOptimizedImage(getSecureUrl(content.pillars[id].image), 400)} alt="" className="w-full h-full object-cover transition-transform duration-[2s] group-hover:scale-125" /> : <ImagePlaceholder label={content.pillars[id]?.label} />}
                ...
                <p className="text-primary-200 text-sm mb-8 leading-relaxed opacity-80 text-center italic">{content.pillars[id]?.desc}</p>
      ...
      {/* 4. CITATION (SOFT ZOOM EFFECT) */}
      <section className="py-32 px-4 text-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 1.5, ease: "easeOut" }} className="max-w-5xl mx-auto bg-gold-50 rounded-[4rem] p-16 md:p-24 relative overflow-hidden shadow-2xl border border-gold-100/50">
          <Quote className="absolute top-10 left-10 text-gold-200 w-32 h-32 opacity-30 -scale-x-100" />
          <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-4xl md:text-7xl font-serif font-bold text-primary-900 mb-10 italic drop-shadow-sm leading-tight">
            {content.quote?.title}
          </motion.h2>
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.8 }} className="text-2xl md:text-3xl text-gray-700 max-w-3xl mx-auto leading-relaxed italic font-light opacity-90">
            "{content.quote?.text}"
          </motion.p>
        </motion.div>
      </section>
      ...
      <footer className="bg-white border-t border-gray-100 py-24 px-4">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-16 text-center md:text-left">
            ...
            <motion.div variants={fadeInUp} className="bg-primary-950 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
              ...
              <p className="text-[10px] font-black uppercase text-gold-500 mb-4 tracking-widest leading-none">Assistance</p>

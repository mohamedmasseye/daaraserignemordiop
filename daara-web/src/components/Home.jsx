  ...
  import axios from 'axios'; // J'ai ajouté l'import axios qui manquait peut-être
  import { 
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden relative border-2 border-gold-500/30">
          ...
          <div className="h-48 relative">
            <img src={event.image || "https://images.unsplash.com/photo-1555431189-0fabf2667795?auto=format&fit=crop&q=80"} alt={event.title} className="w-full h-full object-cover"/>
  ...
  export default function Home() {
    ...
    // --- NOUVELLE LOGIQUE POPUP (1 FOIS PAR JOUR) ---
    useEffect(() => {
      const checkEvents = async () => {
        try {
          ...
          if (nextEvent) {
              ...
              const todayStr = new Date().toDateString(); // "Sun Dec 14 2025"
              // On regarde ce qu'on a stocké (ID + Date)
              const storedData = JSON.parse(localStorage.getItem('home_popup_log') || '{}');

              // On affiche SI : Nouvel événement OU Même événement mais nouvelle date
              if (storedData.eventId !== nextEvent._id || storedData.lastSeenDate !== todayStr) {
                  // Petit délai pour laisser le site charger
    ...
    // Fonction de fermeture qui sauvegarde la date du jour
        ...
        <AnimatePresence>
          {showPopup && featuredEvent && (
              <EventPopup 
                  ...
                  onBook={() => {
                      handleClosePopup(); // On ferme la popup avant de naviguer
                      navigate('/evenements'); // Redirection vers la page agenda
                  }} 
              />
          )}
        </AnimatePresence>
        ...
        <div className="relative h-[90vh] min-h-[600px] flex items-center justify-center overflow-hidden bg-primary-900">
          <AnimatePresence mode='wait'>
            <motion.div key={content.slides[currentSlide].id} initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.5 }} className="absolute inset-0 z-0">
              <img src={content.slides[currentSlide].image} alt="Hero" className="w-full h-full object-cover"/>
          ...
          <div className="relative z-10 max-w-5xl mx-auto px-4 text-center space-y-6">
            <AnimatePresence mode='wait'>
              <motion.div key={content.slides[currentSlide].id} initial={{ opacity: 0, y: 30, filter: "blur(10px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={{ opacity: 0, y: -30, filter: "blur(10px)" }} transition={{ duration: 0.8, delay: 0.2 }}>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="grid md:grid-cols-2 gap-12 items-center">
            <motion.div variants={fadeInUp} className="relative">
              ...
              <div className="relative rounded-[2rem] overflow-hidden shadow-2xl h-[500px] bg-primary-900 group">
                <img src={content.about.image} alt="Serigne Mor Diop" className="w-full h-full object-cover opacity-90 group-hover:scale-105 transition-transform duration-700"/>
            ...
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid md:grid-cols-3 gap-8">
              ...
              <motion.div variants={fadeInUp} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden hover:bg-white/10 transition-colors group cursor-pointer" onClick={() => navigate('/boutique')}>
                <div className="h-48 bg-gray-800 relative overflow-hidden">
                    <img src={content.pillars.shopImage} alt="Boutique" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"/>
              ...
              <motion.div variants={fadeInUp} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden hover:bg-white/10 transition-colors group cursor-pointer" onClick={() => navigate('/livres')}>
                <div className="h-48 bg-gray-800 relative overflow-hidden">
                    <img src={content.pillars.libraryImage} alt="Livres" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"/>
                    <div className="absolute inset-0 bg-primary-900/40 group-hover:bg-primary-900/20 transition-colors"></div>
              ...
              <motion.div variants={fadeInUp} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden hover:bg-white/10 transition-colors group cursor-pointer" onClick={() => navigate('/podcast')}>
                <div className="h-48 bg-gray-800 relative overflow-hidden">
                    <img src={content.pillars.mediaImage} alt="Media" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"/>
                    <div className="absolute inset-0 bg-primary-900/40 group-hover:bg-primary-900/20 transition-colors"></div>

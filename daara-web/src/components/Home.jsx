...
const INSTALL_STEPS = {
  safari: [
    { title: "Étape 1", desc: "Appuyez sur le bouton 'Partager' en bas de Safari.", img: "/safari_step1.png", icon: <Share className="text-blue-500" /> },
...
const PWAInstallGuide = ({ isOpen, onClose }) => {
  const [browser, setBrowser] = useState('safari'); // 'safari' ou 'google'
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl relative">
        ...
        <div className="p-8">
          ...
          <div className="text-center mb-6">
            <h3 className="text-xl font-bold text-primary-900">{INSTALL_STEPS[browser][step].title}</h3>
            <p className="text-gray-500 text-xs mt-2 px-4 leading-relaxed">{INSTALL_STEPS[browser][step].desc}</p>
          </div>

          {/* Zone Image Responsive */}
          <div className="relative h-64 bg-gray-900 rounded-[2rem] border-[6px] border-gray-800 shadow-xl overflow-hidden mb-8 flex items-center justify-center">
             <motion.img 
                ...
                src={INSTALL_STEPS[browser][step].img} 
                className="w-full h-full object-contain" 
                alt="Instruction"
                onError={(e) => { e.target.src = "https://via.placeholder.com/300x500?text=Image+Indisponible"; }}
             />
             <div className="absolute top-4 left-4 bg-white/10 backdrop-blur-md p-2 rounded-xl">{INSTALL_STEPS[browser][step].icon}</div>
          </div>
...
export default function Home() {
  ...
  const [showInstallBanner, setShowInstallBanner] = useState(true); // ✅ État pour la bannière
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resContent, resEvents] = await Promise.all([API.get('/api/home-content'), API.get('/api/events')]);
        if (resContent.data) setContent(prev => ({ ...prev, ...resContent.data, about: { ...prev.about, ...resContent.data.about }, pillars: { ...prev.pillars, ...resContent.data.pillars } }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 overflow-x-hidden">
      ...
      {/* ✅ BANNIÈRE D'INSTALLATION (TOUT EN HAUT) */}
      <AnimatePresence>
        {showInstallBanner && (
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="grid md:grid-cols-2 gap-12 items-center">
          <motion.div variants={fadeInUp} className="relative">
            ...
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl h-[500px] bg-primary-900 group flex items-center justify-center">
              {getSecureUrl(content.about.image) ? <img src={getOptimizedImage(getSecureUrl(content.about.image), 600)} alt="Serigne Mor" className="w-full h-full object-cover opacity-90 group-hover:scale-105 transition-transform duration-700" loading="lazy" /> : <ImagePlaceholder label="Portrait" />}
          ...
          <motion.div variants={fadeInUp} className="space-y-6">
            ...
            <h2 className="text-4xl md:text-5xl font-serif font-bold text-primary-900 leading-tight">{content.about.title1} <span className="text-gold-500">{content.about.highlight1}</span> {content.about.title2} <span className="text-gold-500">{content.about.highlight2}</span>.</h2>
            ...
            {content.about.bioPdf && (
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setIsBioOpen(true)} className="group flex items-center gap-4 bg-white border-2 border-primary-900/5 px-8 py-4 rounded-2xl shadow-sm hover:shadow-xl hover:border-gold-500/30 transition-all duration-300">
                <div className="flex flex-col items-center"><span className="text-primary-900 font-black uppercase tracking-tighter text-sm mb-0.5">Voir plus</span><motion.div animate={{ y: [0, 4, 0] }} transition={{ repeat: Infinity, duration: 1.5 }}><ChevronDown size={18} className="text-gold-500" /></motion.div></div>
      ...
      <section className="bg-primary-900 text-white py-24 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-serif font-bold mb-4">Explorez le Daara</h2><p className="text-primary-200 text-lg">Services et ressources spirituelles.</p></motion.div>
          <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid md:grid-cols-3 gap-8">
            {['p1', 'p2', 'p3'].map((id) => (
              <motion.div key={id} variants={fadeInUp} whileHover={{ y: -10 }} onClick={() => navigate(content.pillars[id]?.link)} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden hover:bg-white/10 transition-colors group cursor-pointer">
                <div className="h-48 bg-gray-800 relative overflow-hidden flex items-center justify-center">
                    {getSecureUrl(content.pillars[id]?.image) ? <img src={getOptimizedImage(getSecureUrl(content.pillars[id].image), 400)} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" /> : <ImagePlaceholder label={content.pillars[id]?.label} />}
                    <div className="absolute top-4 right-4 bg-gold-500 text-primary-900 p-2 rounded-full shadow-lg">{id === 'p1' ? <Calendar size={20} /> : id === 'p2' ? <BookOpen size={20} /> : <PlayCircle size={20} />}</div>
                </div>
      ...
      <section className="py-24 px-4">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="max-w-5xl mx-auto bg-gold-50 rounded-[3rem] p-8 md:p-16 text-center relative overflow-hidden shadow-xl">
          ...
          <div className="relative z-10"><h2 className="text-3xl md:text-5xl font-serif font-bold text-primary-900 mb-8 italic">{content.quote.title}</h2><p className="text-xl md:text-2xl text-gray-700 font-medium mb-10 max-w-2xl mx-auto leading-relaxed">{content.quote.text}</p></div>
        </motion.div>
      </section>
      ...
      <footer className="bg-white border-t border-gray-100 py-16 px-4">
        <div className="max-w-7xl mx-auto text-center md:text-left">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              <motion.div variants={fadeInUp}><h4 className="font-bold text-lg text-primary-900 mb-4 flex items-center gap-2 justify-center md:justify-start"><MapPin className="text-gold-500"/> Adresse</h4><p className="text-gray-600 whitespace-pre-line">{content.info.address}</p></motion.div>
              <motion.div variants={fadeInUp}><h4 className="font-bold text-lg text-primary-900 mb-4 flex items-center gap-2 justify-center md:justify-start"><Clock className="text-gold-500"/> Horaires</h4><p className="text-gray-600 whitespace-pre-line">{content.info.hours}</p></motion.div>
              <motion.div variants={fadeInUp}><h4 className="font-bold text-lg text-primary-900 mb-4 flex items-center gap-2 justify-center md:justify-start"><Calendar className="text-gold-500"/> Prochain Gamou</h4><p className="text-gray-600 whitespace-pre-line"><span className="text-sm text-gold-600 font-bold">{content.info.nextGamou}</span></p></motion.div>

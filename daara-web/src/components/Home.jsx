...
import { 
  ...
  ShoppingBag, Image as ImageIcon, Bell, FileText, ChevronDown, Loader, Download, Share, PlusSquare, Smartphone
} from 'lucide-react';
...
// --- ✅ COMPOSANT GUIDE INSTALLATION PWA (STYLE POCKET OPTION) ---
  ...
  const steps = [
    {
      id: 1,
      title: "Étape 1",
      desc: "Cliquez sur le bouton 'Partager' en bas de votre navigateur Safari.",
      icon: <Share className="text-blue-500" size={32} />
    },
    {
      id: 2,
      title: "Étape 2",
      desc: "Faites défiler vers le bas et appuyez sur 'Sur l'écran d'accueil'.",
      icon: <PlusSquare className="text-primary-900" size={32} />
    },
    {
      id: 3,
      title: "Étape 3",
      desc: "Appuyez sur 'Ajouter' en haut à droite. L'application apparaîtra sur votre téléphone.",
      icon: <Smartphone className="text-gold-500" size={32} />
    }
  ];
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-primary-950/90 backdrop-blur-md">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl relative">
        <button onClick={onClose} className="absolute top-6 right-6 p-2 bg-gray-100 rounded-full text-gray-400 hover:text-red-500 transition-colors"><X size={20}/></button>
        ...
        <div className="p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-primary-900 mb-2">Installer Daara App</h3>
          <p className="text-gray-400 text-[10px] uppercase font-black tracking-widest mb-8">Guide d'installation rapide</p>

          <div className="relative h-64 bg-gray-900 rounded-[2rem] border-[6px] border-gray-800 shadow-xl overflow-hidden flex flex-col items-center justify-center p-6 text-white mb-8">
             <motion.div key={step} initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="flex flex-col items-center">
                <div className="p-4 bg-white/10 rounded-full mb-4">{steps[step-1].icon}</div>
                <h4 className="font-bold text-lg mb-2">{steps[step-1].title}</h4>
                <p className="text-xs text-primary-200 leading-relaxed px-4">{steps[step-1].desc}</p>
             </motion.div>
             <div className="absolute bottom-6 flex gap-2">
                {[1,2,3].map(i => <div key={i} className={`h-1.5 rounded-full transition-all ${step === i ? 'w-6 bg-gold-500' : 'w-2 bg-white/20'}`} />)}
             </div>
          </div>

          <div className="flex gap-4">
            <button disabled={step === 1} onClick={() => setStep(s => s - 1)} className="flex-1 py-4 border border-gray-100 rounded-2xl text-gray-400 font-bold disabled:opacity-0 transition-all"><ChevronLeft className="mx-auto" /></button>
            {step < 3 ? (
...
export default function Home() {
  ...
  const [isPWAOpen, setIsPWAOpen] = useState(false); // ✅ État pour le guide PWA
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        if (res.data) setContent(prev => ({ ...prev, ...res.data, about: { ...prev.about, ...res.data.about }, pillars: { ...prev.pillars, ...res.data.pillars } }));
  ...
  useEffect(() => {
    const checkEvents = async () => {
      try {
        ...
        const upcoming = (res.data || []).filter(e => new Date(e.date) > new Date()).sort((a, b) => new Date(a.date) - new Date(b.date));
        if (upcoming[0]) {
            setFeaturedEvent(upcoming[0]);
            const todayStr = new Date().toDateString();
            const storedData = JSON.parse(localStorage.getItem('home_popup_log') || '{}');
            if (storedData.eventId !== upcoming[0]._id || storedData.lastSeenDate !== todayStr) setTimeout(() => setShowPopup(true), 2500); 
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 overflow-x-hidden">
      ...
      {/* ✅ NOUVEAU : BANDEAU INSTALLATION DAARA APP */}
      <div className="bg-primary-950 text-white py-2.5 px-4 md:px-8 flex items-center justify-between border-b border-white/5 sticky top-0 z-[60] shadow-2xl">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-[9px] md:text-[10px] text-primary-400 font-bold uppercase tracking-widest mt-1">App for iOS</p>
          </div>
        </div>

...
export default function AdminGallery() {
  ...
  const fetchMedia = async () => {
    try {
      const res = await axios.get('http://https://daara-app.onrender.com/api/media');
    ...
    if (window.confirm(`⚠️ Voulez-vous vraiment supprimer ces ${selectedIds.length} éléments définitivement ?`)) {
      ...
      try {
        ...
        for (const id of selectedIds) {
          await axios.delete(`http://https://daara-app.onrender.com/api/media/${id}`);
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        await axios.post('http://https://daara-app.onrender.com/api/media', data, {

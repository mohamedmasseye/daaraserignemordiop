...
import { Trash2, Plus, FileText, Loader, Search, Edit2, Upload, Image as ImageIcon, File } from 'lucide-react';
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => setIsFormVisible(!isFormVisible)} className={`${isFormVisible ? 'bg-gray-500' : 'bg-emerald-600 hover:bg-emerald-700'} text-white px-6 py-3 rounded-xl font-bold shadow-lg transition flex items-center gap-2`}>
          {isFormVisible ? 'Fermer' : <><Plus size={20}/> Nouvel Article</>}
        </button>
      </div>
      ...
      {isFormVisible && (
        <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 mb-10 animate-fade-in-down">
          <h2 className="text-xl font-bold mb-6 text-emerald-800 flex items-center gap-2">
            <div className="p-2 bg-emerald-100 rounded-lg"><FileText size={24}/></div>
            Rédiger un article
          </h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                  ...
                  <input type="text" placeholder="Ex: Résumé de la conférence..." required className="w-full p-3 bg-gray-50 border rounded-xl outline-emerald-500" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50 hover:bg-white transition cursor-pointer relative">
                    <label className="text-xs font-bold text-gray-500 block mb-2 text-center">Image de couverture</label>
                ...
                <div className="p-4 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50 hover:bg-white transition cursor-pointer relative">
                    <label className="text-xs font-bold text-gray-500 block mb-2 text-center">Document PDF (Optionnel)</label>
            ...
            <div className="space-y-4 flex flex-col h-full">
              ...
              <button type="submit" disabled={isUploading} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold shadow-md hover:bg-emerald-700 transition flex justify-center items-center gap-2">
                 {isUploading ? <><Loader size={20} className="animate-spin"/> Publication...</> : "Publier l'article"}
              </button>
            </div>
          </form>
        </div>
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {posts.filter(p => p.title.toLowerCase().includes(searchTerm.toLowerCase())).map((post) => (
              <tr key={post._id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-6 py-4 text-right"><button onClick={() => handleDelete(post._id)} className="p-2 text-gray-400 hover:text-red-600 bg-gray-50 hover:bg-red-50 rounded-lg transition"><Trash2 size={18} /></button></td>
              </tr>
            ))}
          </tbody>
        </table>

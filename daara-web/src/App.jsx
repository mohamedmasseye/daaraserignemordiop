...
// --- DONNÉES DU GUIDE PWA (SORTIES POUR ÉVITER LES CRASHS) ---
const PWA_STEPS = {
  safari: [
...
const PWAHeader = ({ onInstallClick }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-[#050b15] text-white py-2.5 px-4 md:px-8 flex items-center justify-between border-b border-gold-500/20 relative z-[100] shadow-2xl"
    >
      ...
      <div className="flex items-center gap-3">
        <button onClick={onInstallClick} className="bg-gold-500 text-primary-950 px-5 py-1.5 rounded-full font-black text-[10px] uppercase hover:bg-white transition-all shadow-lg flex items-center gap-2">
          Installer <Download size={12} />
        </button>
        <button onClick={() => { setIsVisible(false); localStorage.setItem('daara_pwa_dismissed', 'true'); }} className="p-1 hover:bg-white/10 rounded-full text-gray-500"><X size={18}/></button>
      </div>
    </motion.div>
  );
};

// --- 2. COMPOSANT GUIDE INSTALLATION ---
const PWAInstallGuide = ({ isOpen, onClose }) => {
  const [browser, setBrowser] = useState('safari'); 
  const [step, setStep] = useState(0);
  if (!isOpen) return null;

  const currentData = PWA_STEPS[browser][step];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-white flex flex-col">
      <div className="p-4 md:p-6 flex justify-between items-center bg-white border-b border-gray-100">
        <div className="flex items-center gap-3"><img src="/logo.png" className="w-8 h-8 object-contain" alt=""/><h3 className="font-black text-primary-900 uppercase text-xs">Installation Daara App</h3></div>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center text-center">
          <motion.div key={`${browser}-${step}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <div className="inline-flex items-center gap-2 bg-gold-50 text-gold-600 px-4 py-1.5 rounded-full text-[10px] font-black uppercase mb-4">{currentData.icon} {currentData.title}</div>
            <h4 className="text-xl font-bold text-primary-900 px-4 leading-tight">{currentData.desc}</h4>
          </motion.div>
          <div className="w-full max-w-[280px] aspect-[9/16] bg-gray-100 rounded-[3rem] border-[8px] border-primary-900 shadow-2xl overflow-hidden mb-10">
             <motion.img key={`img-${browser}-${step}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} src={currentData.img} className="w-full h-full object-cover" onError={(e) => {e.target.src="https://via.placeholder.com/600x1200?text=Image+Instructions"}} />
          </div>
      </div>
      <div className="p-6 bg-white border-t border-gray-100 flex items-center justify-between gap-6 shrink-0">
          ...
          {step < 2 ? (
            <button onClick={() => setStep(s => s + 1)} className="flex-1 py-4 bg-primary-900 text-white rounded-2xl font-black uppercase text-xs shadow-xl">Suivant</button>
          ) : (
            <button onClick={onClose} className="flex-1 py-4 bg-gold-500 text-primary-900 rounded-2xl font-black uppercase text-xs shadow-xl">Terminer</button>
          )}
      </div>
    </motion.div>
  );
...
function AppContent() {
  ...
  return (
    <>
      {/* BANNIÈRE APPELÉE ICI POUR ÊTRE AU-DESSUS DE LA NAV */}
